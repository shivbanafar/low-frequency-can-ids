...
def data_from_tfrecord(tf_filepath, batch_size, repeat_time, shuffle=True):
    ...
    # Matches https://github.com/htn274/CanBus-IDS/blob/master/utils.py
    data = data.repeat()  # infinite repeat; finite repeat causes OutOfRangeError when n_batches * batch_size > dataset_size
    data = data.batch(batch_size)
    data = data.prefetch(1)
    iterator = data.make_one_shot_iterator()
    return iterator.get_next()

def train_eval_iterator(tf_filepath, batch_size, num_samples):
    """
    Initializable iterator over labeled TFRecords for one pass (no shuffle).
    Call sess.run(init_op) before each evaluation so train metrics use a full
    pass over the labeled set and can be compared to validation each time.
    """
    data = tf.data.Dataset.from_tensor_slices(tf_filepath)
    data = data.interleave(
        lambda x: tf.data.TFRecordDataset(x),
        cycle_length=len(tf_filepath),
        block_length=10000,
    )
    data = data.map(read_tfrecord, num_parallel_calls=64)
    ...
    return iterator.initializer, next_batch
