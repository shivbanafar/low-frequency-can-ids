"""
Training script aligned with https://github.com/htn274/CanBus-IDS (train.py).
Uses tensorflow.compat.v1 for TensorFlow 2.x; optional --labels for non–Car-Hacking folder names.
"""
import sys
import os
...
class Model:
    def __init__(
        ...
        self.attack_class_weight = attack_class_weight
        self.beta1_sup = 0.9
        self.beta1 = 0.5
        self.beta2 = 0.9
        self.num_critic = 5
    ...
    def build(self):
        ...
        # Class-weighted cross-entropy: attack examples (class 1) weighted by attack_class_weight.
        # w_a > 1 penalises false negatives more, improving recall on rare attack windows.
        class_weights = tf.constant([1.0, self.attack_class_weight], dtype=tf.float32)
        per_example_weights = tf.reduce_sum(self.y_input * class_weights, axis=1)
        per_example_loss = tf.nn.softmax_cross_entropy_with_logits(
            labels=self.y_input, logits=self.encoder_output_label_
        )
        self.supervised_encoder_loss = tf.reduce_mean(per_example_weights * per_example_loss)
        self.supervised_encoder_optimizer = tf.train.AdamOptimizer(
            learning_rate=self.learning_rate, beta1=self.beta1_sup
        ).minimize(self.supervised_encoder_loss, var_list=en_var)
    ...
    def get_val_acc_initializable(self, val_size, batch_size, sess, init_op, next_tensor):
        """Same metrics as get_val_acc, using a resettable finite iterator (labeled train eval)."""
        sess.run(init_op)
        y_true, y_pred = [], []
        if val_size <= 0:
            return 0.0, 0.0, 0.0, 0.0
        ...
        n_labels = 2
        for j in tqdm.tqdm(range(num_batches)):
            data = sess.run(next_tensor)
            batch_x_l = data['input_features']
            y = np.asarray(data['label']).reshape(-1)
            batch_y_l = np.eye(n_labels)[y].reshape([batch_x_l.shape[0], n_labels])
            batch_pred = sess.run(
                self.output_label,
                feed_dict={self.x_input_l: batch_x_l, self.y_input: batch_y_l, self.keep_prob: 1.0},
            )
            y_pred += batch_pred.tolist()
            y_true += y.tolist()
    ...
    def train(self):
        ...
        self.train_eval_init, self.train_eval_next = train_eval_iterator(
            self.train_label_paths, self.batch_size, self.n_labeled
        )
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(self.n_epochs):
                # Keep LR decay happening even when n_epochs changes.
                # Previously this was hardcoded to epoch 50 (meaning it never triggered for shorter runs).
                if epoch == max(0, self.n_epochs // 2):
                    self.supervised_lr /= 10
                    self.reconstruction_lr /= 10
                    self.regularization_lr /= 10

                # Matches upstream: int(n_labeled / batch_size)
                ...
                if (epoch + 1) % 2 == 0:
                    ...
                    acc_known, precision_known, recall_known, f1_known = self.get_val_acc(
                        self.validation_size, self.batch_size, validation, sess
                    )
                    ...
                    acc_train, _, _, f1_train = self.get_val_acc_initializable(
                        self.n_labeled, self.batch_size, sess, self.train_eval_init, self.train_eval_next
                    )
                    print("Accuracy on labeled train (eval pass): {}".format(acc_train))
                    print("F1 on labeled train (eval pass): {}".format(f1_train))
                    accs_train['known'].append(acc_train)
                    f1s_train['known'].append(f1_train)

                    if f1_known > best_f1:
                        best_f1 = f1_known
                        best_sess = sess
                    elif (epoch + 1) == self.n_epochs and best_sess is not None:
                        sess = best_sess

                    if self.unknown_attack is not None:
                        acc_unknown, precision_unknown, recall_unknown, f1_unknown = self.get_val_acc(
                            self.validation_unknown_size, self.batch_size, self.validation_unknown, sess
                        )
            ...
            with open(log_path + '/sum_val.txt', 'w') as summary:
                ...
                summary.write(json.dumps(accs_train))
                summary.write(json.dumps(f1s_train))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch_size', type=int, default=64, help='Upstream default is 64')
    ...
    parser.add_argument(
        ...
        nargs='+',
        default=None,
        help='Subfolders under data_dir (default: DoS Fuzzy gear RPM Normal). Example: MergedLowFreq Normal',
    )
    parser.add_argument('--is_train', action='store_true')
    parser.add_argument('--attack_class_weight', type=float, default=1.0,
                        help='Weight for attack class in supervised loss (paper uses 25.0). Default 1.0 = unweighted.')
    args = parser.parse_args()

    model = Model(
        model=args.model,
        data_dir=args.data_dir,
        unknown_attack=args.unknown_attack,
        batch_size=args.batch_size,
        n_epochs=args.epochs,
        labels=args.labels,
        attack_class_weight=args.attack_class_weight,
    )
    if args.is_train:
        model.train()
    else:
        if args.res_path is None:
            ...
            if args.unknown_attack is not None:
